...
const Chat = ({ isOpen, onClose }) => {
    ...
    const handleSendMessage = () => {
        if (message.trim()) {
            ...
            setTimeout(() => {
                setMessages(prevMessages => [...prevMessages, { text: 'This is an auto-response', sender: 'bot' }]);
            }, 1000);
        }
    };
    ...
    return (
        <div className="fixed bottom-0 right-0 w-80 h-96 bg-white shadow-lg rounded-t-lg flex flex-col overflow-hidden">
            <div className="text-black px-6 pt-2 pb-1 flex justify-between items-center">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#092053" class="bi bi-x" viewBox="0 0 16 16"
                onClick={onClose} className="text-lg cursor-pointer">
            ...
            <div className="flex-1 p-3 overflow-y-auto">
                {messages.map((msg, index) => (
                    <div key={index} className={`mb-2 p-2 rounded text-[#111C36] text-sm max-w-fit ${msg.sender === 'user' ? 'bg-[#D3E5EC] ml-auto' : 'bg-[#F0F0F0] mr-auto'}`}>
                        {msg.text}
                    </div>
                ))}
            </div>
